...
for(d in seq_along(dep)){
	eq = paste("polym(",ind[i],",","degree=",polyord,",","raw=TRUE)"
	  ...
	  ,"+polym(",ind[i+2],",","degree=",polyord,",","raw=TRUE)"
	  ,sep="")
...
for(d in seq_along(dep)){
	...
	,"+polym(",ind[i],",degree=",polyord2,",","raw=TRUE)",":polym(",ind[i+1],",degree=",polyord2,",","raw=TRUE)"
	,"+polym(",ind[i+1],",degree=",polyord2,",","raw=TRUE)",":polym(",ind[i+2],",degree=",polyord2,",","raw=TRUE)"
	,"+polym(",ind[i],",degree=",polyord2,",","raw=TRUE)",":polym(",ind[i+2],",degree=",polyord2,",","raw=TRUE)"
	 ,sep="")
	lmod<-lm(reformulate(eq,dep[d]),data=MyData)
	var1 = paste(signif(summary(lmod)$r.squared,4),",",signif(summary(lmod)$adj.r.squared,4),",(",signif(lmp(lmod),4),")",sep = "")
      mat2[d,i+1] <-var1
